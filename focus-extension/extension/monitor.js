...
class FocusMonitor {
  ...
  init() {
    ...
    document.getElementById('alertDelay')?.addEventListener('change', e => {
    ...
    if (this.debugEnabled) {
      ...
      console.log('[FocusMonitor][debug] Tip: toggle via localStorage.focusMonitorDebug=\'1\' or add ?debug');
    }
  }
  ...
  pollTracker() {
    ...
    if (feed && feed.videoWidth) {
      if (this.canvas.width !== feed.videoWidth) this.canvas.width = feed.videoWidth;
      if (this.canvas.height !== feed.videoHeight) this.canvas.height = feed.videoHeight;
      
      // Draw frame mirrored horizontally to match user expectation
      this.ctx.save();
      this.ctx.translate(this.canvas.width, 0);
      this.ctx.scale(-1, 1);
      this.ctx.drawImage(feed, 0, 0, this.canvas.width, this.canvas.height);
      this.ctx.restore();
    } else {
      // Fallback clear
      this.ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    ...
    if (this.isCapturingHeadBaseline) {
      ...
      const baselineTimeout = this.headBaselineStartTime && (Date.now() - this.headBaselineStartTime > 3000);
      if (this.headBaselineFrames.length >= 18 || baselineTimeout) {
        ...
        this.isCapturingHeadBaseline = false;
        console.log('[FocusMonitor] Head baseline captured:', this.headBaseline, 'frames:', this.headBaselineFrames.length);
      } else {
        if (this.headPositionEl) {
        ...
        this.drawHeadOverlay('Calibrating head...', '#fbbf24', calDebug);
        return;
      }
    }
    ...
    if (pred && typeof pred.x === 'number' && typeof pred.y === 'number') {
      ...
      this.debugLog({ pred: { x: Math.round(pred.x), y: Math.round(pred.y) }, gazeHistoryLen: this.gazeHistory.length });
      this.analyzeGaze();
      if (this.confidenceEl) {
  ...
  drawHeadOverlay(text, color, debugInfo = null) {
    ...
    this.ctx.font = '600 16px system-ui, -apple-system, Segoe UI, Roboto, Arial';
  ...
  updateUI(metrics, score, faceDetected) {
    if (this.faceDetectedEl) {
      ...
      this.faceDetectedEl.style.color = faceDetected ? '#4ade80' : '#ff6b6b';
    }
    
    if (this.confidenceEl) {
      this.confidenceEl.textContent = faceDetected ? `${Math.round(score * 100)}%` : '--';
    }
  }
  ...
  updateStats() {
    ...
    if (this.distractedTimeEl) {
      this.distractedTimeEl.textContent = this.formatTime(this.distractedMs);
    }
    ...
    const score = totalTracked > 0 ? Math.round((this.focusedMs / totalTracked) * 100) : 100;
  ...
  renderTimeline() {
    ...
    this.timeline.innerHTML = this.timelineData.map(state => 
      `<div class="timeline-segment ${state}"></div>`
    ).join('');
  }
  ...
  analyzeGaze() {
    ...
    const margin = this.sensitivity === 'high' ? 0.05 : this.sensitivity === 'low' ? 0.2 : 0.12;
    ...
    const onScreen = avgX >= minX && avgX <= maxX && avgY >= minY && avgY <= maxY;
